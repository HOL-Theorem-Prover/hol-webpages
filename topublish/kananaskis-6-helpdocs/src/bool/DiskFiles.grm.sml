...
open LrTable
val is_keyword =
fn (T 7) => true | (T 8) => true | (T 9) => true | (T 10) => true | _ => false
...
MlyValue.idpair_list idpair_list,idpair_list1left,_))::rest671) => 
let val result=MlyValue.idpair_list((
...
MlyValue.ID ID,_,_))::_::(_,(_,TMV1left,_))::rest671) => let val 
result=MlyValue.termdecl((ptm_v(ID, NUMBER)))
 in (LrTable.NT 13,(result,TMV1left,RBRACKET1right),rest671) end
| (19,(_,(_,_,RBRACKET1right))::(_,(MlyValue.NUMBER NUMBER2,_,_))::(_,
(MlyValue.NUMBER NUMBER1,_,_))::_::(_,(_,TMC1left,_))::rest671) => 
let val result=MlyValue.termdecl((ptm_c(NUMBER1, NUMBER2)))
...
MlyValue.tmid tmid,_,_))::(_,(_,BACKSLASH1left,_))::rest671) => let 
val result=MlyValue.term_a((abs(tmid, term_a)))
