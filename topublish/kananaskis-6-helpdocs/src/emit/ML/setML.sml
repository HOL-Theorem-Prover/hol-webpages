...
struct
  nonfix POW count MIN_SET MAX_SET SUM_SET SUM_IMAGE CROSS DISJOINT CARD
         BIGINTER BIGUNION IMAGE PSUBSET SUBSET DIFF DELETE INTER UNION
         IN INSERT EMPTY * / div mod + - ^ @ <> > < >= <= := o before;
  ...
  fun MAX_SET EMPTY = raise (Fail "MAX_SET: empty set")
    | MAX_SET (INSERT (e,EMPTY)) = e
    | MAX_SET (INSERT (e1,INSERT (e2,s))) =
        MAX e1 (MAX_SET (INSERT (e2,s)))
