...
struct
  nonfix FEVERY o_f FRANGE RRESTRICT DRESTRICT SUBMAP fdomsub FUNION
  ...
  fun fdomsub FEMPTY k = FEMPTY
    | fdomsub (FUPDATE (fm,(k1,v))) k2 =
        (if k1 = k2 then
           fdomsub fm k2
  ...
  fun SUBMAP FEMPTY f = true
    | SUBMAP (FUPDATE (f,(x,y))) g =
        IN x (FDOM g) andalso
        ((FAPPLY g x = y) andalso SUBMAP (fdomsub f x) (fdomsub g x))
  ...
  fun o_f f FEMPTY = FEMPTY
    | o_f f (FUPDATE (fm,(k,v))) =
        FUPDATE (o_f f (fdomsub fm k),(k,f v))
