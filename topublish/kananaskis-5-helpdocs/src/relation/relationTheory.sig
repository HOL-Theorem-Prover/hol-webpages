...
(*
   ...
   [WFREC_DEF]  Definition
      ...
      |- !R M.
           WFREC R M =
           (\x.
              M (RESTRICT (the_fun R^+ (\f v. M (RESTRICT f R v) v) x) R x)
                x)
   ...
   [approx_def]  Definition
      ...
      |- !R M x f.
           approx R M x f <=> (f = RESTRICT (\y. M (RESTRICT f R y) y) R x)
   ...
   [INDUCTIVE_INVARIANT_ON_WFREC]  Theorem
      ...
      |- !R P M D x.
           WF R /\ INDUCTIVE_INVARIANT_ON R D P M /\ D x ==>
           P x (WFREC R M x)
   
   [INDUCTIVE_INVARIANT_WFREC]  Theorem
      
      |- !R P M.
           WF R /\ INDUCTIVE_INVARIANT R P M ==> !x. P x (WFREC R M x)
   ...
   [RTC_MONOTONE]  Theorem
      ...
      |- !R Q. (!x y. R x y ==> Q x y) ==> !x y. R^* x y ==> Q^* x y
   ...
   [TFL_INDUCTIVE_INVARIANT_ON_WFREC]  Theorem
      ...
      |- !f R D P M x.
           (f = WFREC R M) /\ WF R /\ INDUCTIVE_INVARIANT_ON R D P M /\
           D x ==>
           P x (f x)
   ...
   [TFL_INDUCTIVE_INVARIANT_WFREC]  Theorem
      ...
      |- !f R P M x.
           (f = WFREC R M) /\ WF R /\ INDUCTIVE_INVARIANT R P M ==>
           P x (f x)
