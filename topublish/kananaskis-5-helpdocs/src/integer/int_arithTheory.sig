...
(*
   ...
   [INT_DIVIDES_RELPRIME_MUL]  Theorem
      ...
      |- !p q r.
           (gcd p q = 1) ==> (&p int_divides &q * r <=> &p int_divides r)
   ...
   [bmarker_rewrites]  Theorem
      ...
      |- !p q r.
           ...
           (q /\ int_arith$bmarker p /\ r <=>
            int_arith$bmarker p /\ q /\ r) /\
           ((int_arith$bmarker p /\ q) /\ r <=>
            int_arith$bmarker p /\ q /\ r)
   ...
   [bot_and_greaters]  Theorem
      ...
      |- !P d x0.
           (!x. P x ==> P (x + d)) /\ P x0 ==> !c. 0 < c ==> P (x0 + c * d)
   ...
   [elim_eq_coeffs]  Theorem
      ...
      |- !m x y.
           m <> 0 ==> ((&m * x = y) <=> &m int_divides y /\ (x = y / &m))
